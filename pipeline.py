...
from datasets_pcm import NOCSDatasetPCM, Shapenet6DDatasetPCM, TOYLDatasetPCM
...
from utils.pcd import nn_correspondences, nn_correspondences_via_patch_corrs_map
...
############ GETTER METHODS ##################
    ...
    def get_callbacks(self):
        ...
        bar_callback = TQDMProgressBar(refresh_rate=1)
    ...
    def get_model(self) -> Net: 
        ...
        return Net(self.args, self.args.device)
    ...
    def get_dataset(self, eval:bool) -> torch.utils.data.Dataset:
        ...
        if dataset_name == 'nocs':
            return NOCSDatasetPCM(self.args, eval)
        elif dataset_name == 'shapenet6d':
            return Shapenet6DDatasetPCM(self.args, eval)
        elif dataset_name == 'toyl':
            return TOYLDatasetPCM(self.args, eval)
        else:
...
################## VALIDATION ##########################    
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i_b in range(BS):
            ...
            if self.is_detection_valid(results, batch, i_b):
                ...
                if pred_corrs != None:
                    pred_pose, orig_pred_corrs = self.get_pose(batch, pred_corrs, idx=i_b)
                    ...
                    if self.args.viz_valid and ((i_b % 10) == 0):
                    #    pred_corrs = rescale_coords(pred_corrs, (FH,FW), (IH,IW))
                        self.valid_visualization(batch, results, pred_q.clone().numpy(), pred_corrs.cpu().clone().numpy(), orig_pred_corrs.cpu().clone().numpy(), i_b)
                else:
                    print("Nothing found :(")
    ...
    def valid_visualization(self, batch: dict, loss_results: dict, pred_pose: np.ndarray, pred_corrs: np.ndarray, orig_pred_corrs: np.ndarray, batch_idx: int, test:bool=False):
        '''
        ...
        orig_rgb_a = batch['anchor']['orig_rgb'][batch_idx].cpu().squeeze().clone().numpy().transpose(1,2,0)
        orig_rgb_q = batch['query']['orig_rgb'][batch_idx].cpu().squeeze().clone().numpy().transpose(1,2,0)
        orig_gt_mask_a = batch['anchor']['orig_mask'][batch_idx].cpu().squeeze().numpy()
        orig_gt_mask_q = batch['query']['orig_mask'][batch_idx].cpu().squeeze().numpy()
        orig_pred_mask_a = batch['anchor']['pred_mask'][batch_idx].cpu().squeeze().numpy()
        orig_pred_mask_q = batch['query']['pred_mask'][batch_idx].cpu().squeeze().numpy()
        orig_gt_corrs = batch['all_corrs'][batch_idx].clone().cpu().numpy()
        ...
        # viz.pred_mask(orig_rgb_a, orig_rgb_q, orig_gt_mask_a, orig_gt_mask_q, orig_pred_mask_a, orig_pred_mask_q, mask_logits_a, mask_logits_q, out_path + '_mask.png') 
        # viz.corr_set(orig_rgb_a, orig_rgb_q, orig_gt_corrs, orig_pred_corrs, out_path+'_corrs.png')
        # viz.corr_set(rgb_a, rgb_q, gt_corrs, pred_corrs, out_path+'_corrs2.png')
        ...
        viz.pred_pose_nocs(item_a['rgb'], item_q['rgb'], gt_pose, pred_pose, dataset.K, obj_model, out_path+'_pose.png')

    def patch_corr_map_visualization(self, batch: dict, net_output: dict, batch_idx: int, accuracy_threshold: float = 1.0):
        patch_corrs_map = net_output['patch_corrs_map'][batch_idx].clone()
        patch_corrs_map = F.softmax(patch_corrs_map, dim=1)
        gt_patch_corrs_map = batch['anchor']['patch_corrs_map'][batch_idx].clone()
        grid_size = int(patch_corrs_map.shape[0]**0.5)
        img_a = (batch['anchor']['rgb'][batch_idx].clone().permute(1, 2, 0).cpu().numpy()*255).astype(np.uint8)
        img_q = (batch['query']['rgb'][batch_idx].clone().permute(1, 2, 0).cpu().numpy()*255).astype(np.uint8)
        img_h, img_w = img_a.shape[:2]
        instance_id = batch['instance_id'][batch_idx]

        for i in range(patch_corrs_map.shape[0]):
            corrs_map = patch_corrs_map[i,:-1]
            gt_corrs_map = gt_patch_corrs_map[i,:-1]
            if not torch.all(corrs_map == 0):
                topk_idx = torch.where(corrs_map > 0.04)[0]
                gt_idx = torch.where(gt_corrs_map > 0.)[0]
                
                topk_idx = topk_idx.cpu().numpy()
                gt_idx = gt_idx.cpu().numpy()

                if len(gt_idx) > 0 and len(topk_idx) > 0:
                    correct_predictions = np.sum(np.isin(topk_idx, gt_idx))
                    accuracy = correct_predictions / min(len(topk_idx), len(gt_idx))
                    
                    # iou matching
                    # intersection = len(np.intersect1d(topk_idx, gt_idx))
                    # union = len(np.union1d(topk_idx, gt_idx))
                    # accuracy = intersection / union if union > 0 else 0.0
                    
                    if accuracy >= accuracy_threshold:
                        alpha = 0.5
                        mask_a = torch.zeros(grid_size**2, device='cpu')
                        mask_a[i] = 1.
                        mask_a = mask_a.view(grid_size, grid_size)
                        mask_a = torch.nn.functional.interpolate(mask_a.unsqueeze(0).unsqueeze(0), (img_h,img_w), mode='nearest').squeeze(0).squeeze(0)
                        mask_a = (mask_a.numpy()* 255).astype(np.uint8)
                        heatmap_a = cv2.applyColorMap(mask_a, cv2.COLORMAP_JET)
                        overlay_a = cv2.addWeighted(img_a, 1 - alpha, heatmap_a, alpha, 0)

                        mask_q = torch.zeros(grid_size**2, device='cpu')
                        mask_q[topk_idx] = 1.
                        mask_q = mask_q.view(grid_size, grid_size)
                        mask_q = torch.nn.functional.interpolate(mask_q.unsqueeze(0).unsqueeze(0), (img_h,img_w), mode='nearest').squeeze(0).squeeze(0)
                        mask_q = (mask_q.numpy()* 255).astype(np.uint8)
                        heatmap_q = cv2.applyColorMap(mask_q, cv2.COLORMAP_JET)
                        overlay_q = cv2.addWeighted(img_q, 1 - alpha, heatmap_q, alpha, 0)

                        mask_q_gt = torch.zeros(grid_size**2, device='cpu')
                        mask_q_gt[gt_idx] = 1.
                        mask_q_gt = mask_q_gt.view(grid_size, grid_size)
                        mask_q_gt = torch.nn.functional.interpolate(mask_q_gt.unsqueeze(0).unsqueeze(0), (img_h,img_w), mode='nearest').squeeze(0).squeeze(0)
                        mask_q_gt = (mask_q_gt.numpy()* 255).astype(np.uint8)
                        heatmap_q_gt = cv2.applyColorMap(mask_q_gt, cv2.COLORMAP_JET)
                        overlay_q_gt = cv2.addWeighted(img_q, 1 - alpha, heatmap_q_gt, alpha, 0)

                        images = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in [overlay_a, overlay_q, overlay_q_gt]]
                        titles = ['anchor', 'predicted map', 'gorund truth map']
                        ...
                        out_dir = f'{self.args.tmp.results_out}/viz_patch_corr_map/'
                        if not os.path.exists(out_dir):
                            os.makedirs(out_dir)
                        out_path = out_dir + f'{self.args.dataset.test.name}_{self.args.dataset.test.split}_epoch{self.current_epoch}_{instance_id}_{self.args.test.mask}_idx{i:02d}.png'
...
################ TEST ################################
        ...
        for i_b in range(BS):
            ...
            if self.is_detection_valid(results, batch, i_b):
                ...
                if pred_corrs != None:
                    ...
                    if self.args.viz_test and batch['instance_id'][i_b] in self.test_dataset.tracked_instances:
                        self.valid_visualization(batch, results, pred_q.clone().numpy(), pred_corrs.cpu().clone().numpy(), orig_pred_corrs.cpu().clone().numpy(), i_b, test=True)
                        # self.patch_corr_map_visualization(batch, outputs, i_b)
                else:
    ...
    def get_featmap_corrs(self, batch: dict, net_output: dict, results: dict, idx: int) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # normal case, use predicted mask
            ...
            if mask_ai.shape[0] != NH or mask_ai.shape[1] != NW:
                mask_ai = F.interpolate(mask_ai.unsqueeze(0).unsqueeze(0).to(torch.float), (NH,NW), mode='nearest').squeeze().to(torch.int)
                mask_qi = F.interpolate(mask_qi.unsqueeze(0).unsqueeze(0).to(torch.float), (NH,NW), mode='nearest').squeeze().to(torch.int)
        
        featmap_ai, featmap_qi = net_output['featmap_a'][idx].clone(), net_output['featmap_q'][idx].clone()
        
        patch_corrs_map = net_output['patch_corrs_map'][idx].clone()
        patch_corrs_map = F.softmax(patch_corrs_map, dim=1)
        pred_corrs = nn_correspondences_via_patch_corrs_map(featmap_ai, featmap_qi, mask_ai, mask_qi, self.args.test.dist_th, self.args.test.n_corrs, self.args.test.src_sampling, self.corrs_device, patch_corrs_map)
        if pred_corrs is None:
            print(idx, "No valid correspondences found")
    ...
    def get_pose(self, batch : dict, corrs : Tensor, idx : int) -> Tuple[Tensor, Tensor]:
        '''
        ...
        corrs_a = corrs_a + batch['anchor']['pred_boxes'][idx][:2].to(corrs_a.device)
        corrs_q = corrs_q + batch['query']['pred_boxes'][idx][:2].to(corrs_q.device)

        orig_pred_corrs = torch.cat([corrs_a, corrs_q], dim=1)
        ...
        return pose4.to(torch.float32), orig_pred_corrs
