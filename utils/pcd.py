...
def project_points(v : np.ndarray, k : np.ndarray) -> np.ndarray:
    ...
    p = np.matmul(k, v.T)
    # p[2] = np.where(np.abs(p[2]) < 1e-6, 1e-6, p[2])
    p[2] = np.maximum(p[2], 1e-6)
    
    p[0] = p[0] / p[2]
    p[1] = p[1] / p[2]

    max_val = np.finfo(np.float16).max   # avoid overflow
...
def nn_correspondences_via_patch_corrs_map(feats1: Tensor, feats2: Tensor, mask1: Tensor, mask2: Tensor, threshold: float, max_corrs: int, subsample_source:int, corrs_device: str, patch_corrs_map: Tensor) -> Tensor:
    final_corrs = []
    grid_size = int(patch_corrs_map.shape[0]**0.5)
    _,H,W = feats1.shape
    for i in range(patch_corrs_map.shape[0]):
        mask_a = torch.zeros(grid_size**2, device=feats1.device)
        mask_a[i] = 1.
        mask_a = mask_a.view(grid_size, grid_size)
        mask_a = torch.nn.functional.interpolate(mask_a.unsqueeze(0).unsqueeze(0), (H,W), mode='nearest').squeeze(0).squeeze(0)
        roi1 = (mask1.bool() & mask_a.bool()).long()

        corrs_map = patch_corrs_map[i,:]

        mask_q = torch.zeros(grid_size**2, device=feats1.device)
        if not torch.all(corrs_map == 0):
            # _, mask_idx = torch.topk(corrs_map, 8)
            mask_idx = torch.where(corrs_map > 0.04)[0]
            mask_q[mask_idx] = 1.
        mask_q = mask_q.view(grid_size, grid_size)
        mask_q = torch.nn.functional.interpolate(mask_q.unsqueeze(0).unsqueeze(0), (H,W), mode='nearest').squeeze(0).squeeze(0)
        roi2 = (mask2.bool() & mask_q.bool()).long()
        if torch.any(roi1) and torch.any(roi2):
            corrs = nn_correspondences(feats1, feats2, roi1, roi2, threshold, max_corrs, subsample_source, corrs_device)
            if corrs is not None:
                final_corrs.append(corrs)

    if len(final_corrs) > 0:
        final_corrs = torch.cat(final_corrs, dim=0)
        if len(final_corrs) > max_corrs:
            idxs = torch_sample_select(final_corrs, max_corrs)
            final_corrs = final_corrs[idxs]
        final_corrs = final_corrs.to(feats1.device)
    else:
        final_corrs = None
    return final_corrs
