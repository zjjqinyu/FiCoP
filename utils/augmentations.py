...
class horizontal_flip(object):
    ...
    def flip_item(self, item, coords):
        ...
        if 'cropped_pred_mask' in item:
            item['cropped_pred_mask'] = F.hflip(item['cropped_pred_mask'].unsqueeze(0)).squeeze(0)
...
class vertical_flip(object):
    ...
    def flip_item(self, item, coords):
        ...
        if 'cropped_pred_mask' in item:
            item['cropped_pred_mask'] = F.vflip(item['cropped_pred_mask'].unsqueeze(0)).squeeze(0)
...
class resize(object):
    ...
    def resize_item(self, item : dict, coords: Tensor) -> Tuple[dict, Tensor]:
        ...
        if 'cropped_pred_mask' in item:
            item['cropped_pred_mask'] = F.resize(item['cropped_pred_mask'].unsqueeze(0), size=list(self.size), interpolation=F.InterpolationMode.NEAREST).squeeze(0)
