...
def pred_pose_nocs(rgb_a : ndarray, rgb_q : ndarray, gt_pose : ndarray, pred_pose : ndarray, K: ndarray, obj_model : ndarray, out_path : str):
    fig, axs = plt.subplots(nrows=2, ncols=2)
    ...
    # build NOCS style color map
    pts = obj_model.copy().astype(np.float32)
    min_xyz = pts.min(axis=0)
    max_xyz = pts.max(axis=0)
    scale = max_xyz - min_xyz
    scale[scale == 0] = 1e-6  # avoid divide by zero
    pts_norm = (pts - min_xyz) / scale  # → NOCS style [0,1]
    model_colors = np.clip(pts_norm, 0.0, 1.0)
    ...
    valid = np.logical_and(np.logical_and(correct_y2, correct_y1), np.logical_and(correct_x1, correct_x2))
    pts = pts[valid]
    colors_valid = model_colors[valid]
    depths = gt_pcd[valid, 2]
    depth_sort_indices = np.argsort(-depths)
    pts_sorted = pts[depth_sort_indices]
    colors_sorted = colors_valid[depth_sort_indices]
