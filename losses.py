...
class FeatureLoss(torch.nn.Module):
    ...
    def forward(self, batch : dict, net_output : dict) -> Tuple[dict,dict]:
        ...
        pred_map_logits = net_output['patch_corrs_map']
        target_map = batch['anchor']['patch_corrs_map']
        B,N1,N2 = pred_map_logits.shape
        pred_map_logits = pred_map_logits.view(-1, N2)
        target_map = target_map.view(-1, N2)

        num_positive = target_map.sum(dim=0)
        num_negative = len(target_map) - num_positive
        pos_weight = num_negative / (num_positive + 1e-7)
        patch_corrs_loss = F.binary_cross_entropy_with_logits(pred_map_logits, target_map, pos_weight=pos_weight)
        
        # this contains loss to be optimized
        losses = {
            'pos' : pos_loss,
            'neg' : 0.5 * (neg_loss_a + neg_loss_q),
            'patch_corrs': patch_corrs_loss,
        }
        # this contains intermediate results used for metrics
        # iou_a = mask_iou(batch['anchor']['orig_mask'], batch['anchor']['pred_mask'])
        # iou_q = mask_iou(batch['query']['orig_mask'], batch['query']['pred_mask'])
        iou_a = mask_iou(batch['anchor']['mask'], batch['anchor']['cropped_pred_mask'])
        iou_q = mask_iou(batch['query']['mask'], batch['query']['cropped_pred_mask'])

        # pred_map_logits = pred_map_logits.view(B,N1,N2)
        # target_map = target_map.view(B,N1,N2)
        # patch_corrs_top8_acc = self.topk_accuracy(pred_map_logits, target_map, k=8)
        # patch_corrs_top16_acc = self.topk_accuracy(pred_map_logits, target_map, k=16)

        results = {
            'neg_a' : neg_a_idxs,
            'neg_q' : neg_q_idxs,
            'mask_a' : batch['anchor']['cropped_pred_mask'],
            'mask_q' : batch['query']['cropped_pred_mask'],
            'logits_a' : torch.ones_like(batch['anchor']['cropped_pred_mask']),
            'logits_q' : torch.ones_like(batch['query']['cropped_pred_mask']),
            'iou_a' : iou_a,
            'iou_q' : iou_q
        }
